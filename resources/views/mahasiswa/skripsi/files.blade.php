<x-portal-layout :title="'Upload File Skripsi - '.config('app.name')" subtitle="Skripsi">
    <x-slot:sidebar>
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div class="min-w-0">
            <div class="text-xl font-semibold">Upload File Skripsi</div>
            <div class="mt-1 text-sm text-emerald-100/70">Upload file skripsi beserta keterangan, lalu bisa preview, download, dan hapus.</div>
        </div>
    </div>
    ...
    @else
        <div class="mt-5 rounded-2xl bg-white/5 border border-white/10 p-6">
            ...
            <div class="mt-1 font-semibold">{{ $skripsi->judul }}</div>
            <div class="mt-3 text-sm text-emerald-100/70">Status: <span class="font-semibold text-white">{{ strtoupper($skripsi->status) }}</span></div>
        </div>

        <form method="POST" action="{{ route('mahasiswa.skripsi-files.store') }}" enctype="multipart/form-data" class="mt-4 rounded-2xl bg-white/5 border border-white/10 p-6">
            @csrf
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input name="keterangan" value="{{ old('keterangan') }}"
                           ...
                           placeholder="Contoh: Bab 1 revisi" />
                            ...
                            <tr class="hover:bg-white/5">
                                ...
                                <td class="px-6 py-3">
                                    <div class="flex items-center justify-end gap-2 flex-wrap">
                                        <a href="{{ route('mahasiswa.skripsi-files.preview', $f) }}" target="_blank"
                                        ...
                                        <a href="{{ route('mahasiswa.skripsi-files.download', $f) }}"
                                           class="h-9 px-3 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
                                            <i class="fa-solid fa-download"></i>
                                            Download
                                        </a>
                                        <form method="POST" action="{{ route('mahasiswa.skripsi-files.destroy', $f) }}" onsubmit="return confirm('Hapus file ini?');">
                                            @csrf
                                            @method('DELETE')
