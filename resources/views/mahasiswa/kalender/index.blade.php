<x-portal-layout :title="'Kalender Akademik - '.config('app.name')" subtitle="Kalender Akademik">
    ...
    <div class="mt-5 rounded-2xl bg-white/5 border border-white/10 p-5">
        <form method="GET" action="{{ route('mahasiswa.kalender.index') }}" class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('mahasiswa.kalender.index') }}" class="h-11 px-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition text-sm font-medium inline-flex items-center">Reset</a>
            </div>
        </form>
    </div>
