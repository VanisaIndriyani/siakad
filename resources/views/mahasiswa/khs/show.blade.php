<x-portal-layout :title="'Detail KHS - '.config('app.name')" subtitle="Detail KHS">
    ...
    <div class="flex items-center justify-between gap-3 mb-5">
        <div>
            ...
            <div class="text-sm text-emerald-100/70">Semester {{ $khs->semester }} • {{ $khs->tahun_ajaran ?? '-' }}</div>
        </div>
        <a href="{{ route('mahasiswa.khs.index') }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
