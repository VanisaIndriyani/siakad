<x-portal-layout :title="'Tambah Jurnal KKN - '.config('app.name')" subtitle="KKN">
    <x-slot:sidebar>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <a href="{{ route('mahasiswa.kkn.jurnal.index', $kkn) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
            <i class="fa-solid fa-arrow-left"></i>
            <span class="text-sm font-medium">Kembali</span>
        </a>
    </div>

    <form method="POST" action="{{ route('mahasiswa.kkn.jurnal.store', $kkn) }}" class="mt-5 rounded-2xl bg-white/5 border border-white/10 p-5">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <x-text-input id="kegiatan" type="text" name="kegiatan" :value="old('kegiatan')" placeholder="Contoh: Sosialisasi Pendidikan Karakter" required />
                <x-input-error :messages="$errors->get('kegiatan')" />
            </div>
            <div>
                <x-input-label for="lokasi" value="Lokasi" />
                <x-text-input id="lokasi" type="text" name="lokasi" :value="old('lokasi')" placeholder="Contoh: Desa Sidrap, Kec. Tellu Limpoe" />
                <x-input-error :messages="$errors->get('lokasi')" />
            </div>
            <div>
                <x-input-label for="pihak_terkait" value="Pihak Terkait" />
                <x-text-input id="pihak_terkait" type="text" name="pihak_terkait" :value="old('pihak_terkait')" placeholder="Contoh: Kepala Desa, Masyarakat, Siswa SD" />
        ...
        <div class="mt-6 flex items-center justify-end gap-2">
            <a href="{{ route('mahasiswa.kkn.jurnal.index', $kkn) }}" class="h-10 px-5 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
