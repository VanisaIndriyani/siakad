<x-portal-layout :title="'Edit Jurnal KKN - '.config('app.name')" subtitle="KKN">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            ...
            <div class="text-sm text-emerald-100/70">{{ $kkn->posko?->nama_posko ?: 'KKN' }} • {{ $jurnal->tanggal?->format('d F Y') }}</div>
        </div>
    ...
    <form method="POST" action="{{ route('mahasiswa.kkn.jurnal.update', [$kkn, $jurnal]) }}" class="mt-5 rounded-2xl bg-white/5 border border-white/10 p-5">
        @csrf
        @method('PUT')
