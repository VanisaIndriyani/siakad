<x-portal-layout :title="'Jurnal Kegiatan PPL - '.config('app.name')" subtitle="PPL">
    <x-slot:sidebar>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <div class="text-xl font-semibold">Jurnal Kegiatan PPL</div>
            <div class="text-sm text-emerald-100/70">
                {{ $ppl->instansi_nama }} • {{ $ppl->mahasiswa?->nama_lengkap }} ({{ $ppl->mahasiswa?->npm }})
            </div>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            <a href="{{ route('mahasiswa.ppl.show', $ppl) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
                <i class="fa-solid fa-arrow-left"></i>
                <span class="text-sm font-medium">Kembali</span>
            </a>
            <a href="{{ route('mahasiswa.ppl.jurnal.pdf', $ppl) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-blue-500/15 hover:bg-blue-500/20 border border-blue-500/20 text-blue-100 transition">
                <i class="fa-solid fa-file-pdf"></i>
                <span class="text-sm font-medium">Cetak PDF</span>
            </a>
            <a href="{{ route('mahasiswa.ppl.jurnal.create', $ppl) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 transition">
            ...
            <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <a href="{{ route('mahasiswa.ppl.jurnal.edit', [$ppl, $jurnal]) }}" class="h-9 px-3 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition text-sm">
                            <i class="fa-solid fa-pen"></i>
                            <span class="font-medium">Edit</span>
                        </a>
                        <form method="POST" action="{{ route('mahasiswa.ppl.jurnal.destroy', [$ppl, $jurnal]) }}" onsubmit="return confirm('Hapus jurnal kegiatan ini?');">
                            @csrf
                            @method('DELETE')
