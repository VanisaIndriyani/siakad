<x-portal-layout :title="'Daftar Hadir PPL - '.config('app.name')" subtitle="PPL">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <a href="{{ route('mahasiswa.ppl.absensi.pdf', $ppl) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-blue-500/15 hover:bg-blue-500/20 border border-blue-500/20 text-blue-100 transition">
                <i class="fa-solid fa-file-pdf"></i>
                <span class="text-sm font-medium">Cetak PDF</span>
            </a>
            <a href="{{ route('mahasiswa.ppl.absensi.create', $ppl) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 transition">
            ...
            <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <a href="{{ route('mahasiswa.ppl.absensi.edit', [$ppl, $absensi]) }}" class="h-9 px-3 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition text-sm">
                            <i class="fa-solid fa-pen"></i>
                            <span class="font-medium">Edit</span>
                        </a>
                        <form method="POST" action="{{ route('mahasiswa.ppl.absensi.destroy', [$ppl, $absensi]) }}" onsubmit="return confirm('Hapus absensi ini?');">
                            @csrf
                            @method('DELETE')
