<x-portal-layout :title="'Revisi PPL - '.config('app.name')" subtitle="PPL">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div class="min-w-0">
            ...
            <div class="mt-1 text-sm text-emerald-100/70">{{ $ppl->instansi_nama }}</div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('mahasiswa.ppl.revisi.pdf', $ppl) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
                <i class="fa-solid fa-print"></i>
                Print
            </a>
            <a href="{{ route('mahasiswa.ppl.bimbingan', $ppl) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
