<x-portal-layout :title="'Laporan - '.config('app.name')" subtitle="Laporan">
    ...
    <div class="chat-wrap">
        <div class="chat-card">
            <div class="chat-head">
                <div>
                    <div class="title">Laporan Pengajuan</div>
                    <div class="sub">
                        {{ $laporan->judul }} • {{ $jenisLabel }} #{{ $laporan->pengajuan_id }} • Status: {{ $laporan->status === 'open' ? 'Open' : 'Closed' }}
                    </div>
                </div>
                <a href="{{ route('mahasiswa.laporan.index') }}" class="chat-back">
            ...
            <form method="POST" action="{{ route('mahasiswa.laporan.pesan.store', $laporan) }}" class="chat-form">
                @csrf
