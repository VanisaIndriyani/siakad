    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        ...
        <div class="flex gap-2 flex-wrap">
            @if($peserta && $peserta->status_hadir && $kegiatan->sertifikat_aktif)
                <a href="{{ route('dosen.kegiatan.download-sertifikat', $kegiatan) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-sky-600 hover:bg-sky-500 active:bg-sky-700 transition font-medium text-sm">
                    <i class="fa-solid fa-download"></i>
                    Download Sertifikat
                </a>
            @endif
            @if(!$peserta)
                <form method="POST" action="{{ route('dosen.kegiatan.daftar', $kegiatan) }}" onsubmit="return confirm('Daftar sebagai peserta kegiatan ini?')">
                    @csrf
            ...
            @endif
            <a href="{{ route('dosen.kegiatan.index') }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition text-sm">
                ...
                @if($peserta)
                    <div class="space-y-3">
                        ...
                        <div class="pt-2 border-t border-white/10 space-y-2 text-xs">
                            ...
                            @if($peserta->nidn)
                                <div class="flex justify-between">
                                    <span class="text-emerald-100/60">NIDN</span>
                                    <span class="font-medium">{{ $peserta->nidn }}</span>
                                </div>
                            @endif
