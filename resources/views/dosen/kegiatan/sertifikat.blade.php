<x-portal-layout :title="'Sertifikat Saya - '.config('app.name')" subtitle="Kumpulan Sertifikat Kegiatan">
    ...
    @if($pesertas->isEmpty())
        <div class="rounded-2xl bg-white/5 border border-white/10 p-12 text-center">
            ...
            <a href="{{ route('dosen.kegiatan.index') }}" class="h-10 px-5 inline-flex items-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 transition text-sm font-medium">
            ...
            @foreach($pesertas as $p)
                ...
                <div class="rounded-2xl overflow-hidden bg-gradient-to-br from-white/[0.07] to-white/[0.03] border border-white/10 hover:border-sky-500/30 transition group">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        @if($adaFile)
                            <a href="{{ route('dosen.kegiatan.download-sertifikat', $p->kegiatan) }}" class="w-full h-10 inline-flex items-center justify-center gap-2 rounded-xl bg-sky-600 hover:bg-sky-500 active:bg-sky-700 transition text-sm font-medium">
