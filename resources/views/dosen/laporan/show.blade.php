<x-portal-layout :title="'Laporan - '.config('app.name')" subtitle="Laporan">
    ...
    <style>
        ...
        .chat-row.me .chat-bubble { background: rgba(59, 130, 246, 0.12); border-color: rgba(59, 130, 246, 0.25); }
        ...
        .chat-send { height: 44px; padding: 0 16px; border-radius: 14px; border: 1px solid rgba(59, 130, 246, 0.25); background: linear-gradient(to right, #2563eb, #3b82f6); color: #fff; font-weight: 900; font-size: 13px; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; }
    ...
    <div class="chat-wrap">
        <div class="chat-card">
            <div class="chat-head">
                <div>
                    <div class="title">Laporan Mahasiswa</div>
                    <div class="sub">
                        {{ $laporan->mahasiswa?->nama_lengkap ?: '-' }} ({{ $laporan->mahasiswa?->npm ?: '-' }})
                        • {{ $laporan->judul }}
                        • {{ $jenisLabel }} #{{ $laporan->pengajuan_id }}
                        • Status: {{ $laporan->status === 'open' ? 'Open' : 'Closed' }}
                    </div>
                </div>
                <a href="{{ route('dosen.laporan.index') }}" class="chat-back">
            ...
            @if (! $canSend)
                <div class="chat-warn">Laporan sudah ditutup, tidak bisa mengirim pesan.</div>
            @endif

            <div id="chatStream" class="chat-stream">
                @forelse ($laporan->messages->sortBy('id') as $msg)
                    @php
            ...
            <form method="POST" action="{{ route('dosen.laporan.pesan.store', $laporan) }}" class="chat-form">
                @csrf
                <textarea name="pesan" rows="2" {{ $canSend ? '' : 'disabled' }}
                          class="chat-input"
                          placeholder="Tulis balasan...">{{ old('pesan') }}</textarea>
                <button {{ $canSend ? '' : 'disabled' }} class="chat-send">
