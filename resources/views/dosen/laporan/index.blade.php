<x-portal-layout :title="'Laporan - '.config('app.name')" subtitle="Laporan">
    ...
    <div class="mt-5 rounded-2xl bg-white/5 border border-white/10 p-5" x-data="{
        ...
        <div class="flex flex-col lg:flex-row gap-3 items-end lg:items-center">
            <form method="GET" action="{{ route('dosen.laporan.index') }}" class="flex-1 flex flex-col lg:flex-row gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('dosen.laporan.index') }}" class="h-11 px-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition text-sm font-medium inline-flex items-center text-white">Reset</a>
                </div>
            </form>
        </div>

        <form x-ref="bulkForm" method="POST" action="{{ route('dosen.laporan.bulk-delete') }}" @change="onBulkChange($event)"
