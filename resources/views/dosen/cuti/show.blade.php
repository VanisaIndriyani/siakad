<x-portal-layout :title="'Detail Pengajuan Cuti - '.config('app.name')" subtitle="Detail Pengajuan Cuti">
    ...
    <div class="flex items-center justify-between gap-3 mb-5">
        ...
        <a href="{{ route('dosen.cuti.index') }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            @if ($cuti->catatan_admin)
                <div class="rounded-2xl bg-white/5 border border-white/10 p-6">
                    <div class="text-xs font-semibold text-emerald-100/60 uppercase tracking-wider mb-2">Catatan Admin:</div>
        ...
        <div class="rounded-2xl bg-white/5 border border-white/10 p-6">
            <div class="text-lg font-semibold mb-4 text-white">Approval Prodi</div>
            ...
            <form method="POST" action="{{ route('dosen.cuti.status', $cuti) }}" class="space-y-4">
                @csrf
                @method('PATCH')

                <div>
                    <label class="text-sm text-emerald-100/80">Ubah Status (Prodi)</label>
                ...
                <div>
                    <label class="text-sm text-emerald-100/80">Catatan Prodi</label>
                    <textarea name="catatan_prodi" rows="3" class="mt-2 w-full rounded-xl bg-white/5 border border-white/10 focus:border-emerald-400 focus:ring-emerald-400 text-white" placeholder="Masukkan catatan kaprodi/sekprodi...">{{ old('catatan_prodi', $cuti->catatan_prodi) }}</textarea>
                    @error('catatan_prodi') <div class="mt-1 text-xs text-red-200">{{ $message }}</div> @enderror
                </div>
