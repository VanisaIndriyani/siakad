<x-portal-layout :title="'Approve KRS - '.config('app.name')" subtitle="Approve KRS">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            ...
            <div class="text-sm text-emerald-100/70">Pantau dan kelola status approval KRS (khusus Prodi).</div>
        </div>
    </div>
    ...
    <form method="GET" class="mt-5 grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <select name="status" class="h-11 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-400 focus:ring-emerald-400">
            <option value="" @selected(($status ?? 'pending') === '') class="text-black">Semua Status</option>
            @foreach (['pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected'] as $k => $v)
                <option value="{{ $k }}" @selected(($status ?? 'pending') === $k) class="text-black">{{ $v }}</option>
            @endforeach
        </select>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    @if ($row->status_approval === 'pending')
                                        <form method="POST" action="{{ route('dosen.krs.update', $row) }}">
                                            @csrf
                                            @method('PATCH')
