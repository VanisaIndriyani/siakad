<x-portal-layout :title="'Detail Hasil Kuesioner - '.config('app.name')" subtitle="Detail Hasil Kuesioner">
    ...
    <div class="flex items-center justify-between gap-3 mb-5">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.kuesioner.export.excel', $mataKuliah) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/20 transition text-emerald-100">
                <i class="fa-solid fa-file-excel"></i>
                Excel
            </a>
            <a href="{{ route('admin.kuesioner.export.pdf', $mataKuliah) }}" class="h-10 px-4 inline-flex items-center gap-2 rounded-xl bg-red-500/15 hover:bg-red-500/25 border border-red-500/20 transition text-red-100">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
            <div class="text-sm text-emerald-100/70">Dosen 1</div>
            <div class="mt-2 text-lg font-semibold">{{ $mataKuliah->dosen?->nama ?? '-' }}</div>
        </div>
        <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
            <div class="text-sm text-emerald-100/70">Dosen 2</div>
    ...
    <div class="mt-8">
        ...
        <div class="mt-1 text-sm text-emerald-100/70">Ringkasan skor tiap pertanyaan pada mata kuliah ini.</div>
    </div>
    ...
    <div class="mt-8">
        ...
        <div class="mt-1 text-sm text-emerald-100/70">Komentar bersifat opsional, sehingga beberapa respon bisa tidak memiliki catatan.</div>
    </div>
    ...
    <div class="mt-4 space-y-4">
        ...
        @empty
            <div class="rounded-2xl bg-white/5 border border-white/10 p-6 text-center text-emerald-100/70">
                Belum ada komentar atau hasil kuesioner untuk mata kuliah ini.
            </div>
        @endforelse
    </div>
