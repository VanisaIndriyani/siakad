    ...
    <form method="POST" action="{{ route('admin.kegiatan.store') }}" enctype="multipart/form-data" class="rounded-2xl bg-white/5 border border-white/10 p-5 space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input name="sekretaris_panitia_nama" value="{{ old('sekretaris_panitia_nama') }}" placeholder="Contoh: Dra. Hj. Siti Nurhaliza, M.Pd" class="mt-2 w-full h-11 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-400 focus:ring-emerald-400" />
            </div>

            <div>
                <label class="text-sm text-emerald-100/80">NIP Sekretaris Panitia <span class="text-emerald-200/40">(Opsional)</span></label>
                <input name="sekretaris_panitia_nip" value="{{ old('sekretaris_panitia_nip') }}" placeholder="Contoh: 197801012005012002" class="mt-2 w-full h-11 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-400 focus:ring-emerald-400" />
            </div>
