            ...
            <tbody class="divide-y divide-white/10">
                @forelse ($mahasiswa as $row)
                    <tr class="hover:bg-white/5">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    <div class="h-10 w-10 rounded-xl bg-emerald-500/20 border border-emerald-500/20 flex items-center justify-center font-semibold">
                                        {{ mb_substr($row->nama_lengkap, 0, 1) }}
                                    </div>
                                @endif
                                <div class="leading-tight">
                                    <div class="font-medium text-white">{{ $row->nama_lengkap }}</div>
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center rounded-full bg-emerald-500/15 border border-emerald-500/20 px-3 py-1 text-xs text-emerald-100">
                                {{ $row->status_mahasiswa }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.mahasiswa.show', $row) }}" class="h-9 w-9 inline-flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition" title="Detail">
                                    <i class="fa-solid fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.mahasiswa.edit', $row) }}" class="h-9 w-9 inline-flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition" title="Edit">
                                    <i class="fa-solid fa-pen"></i>
                                </a>
                                <form method="POST" action="{{ route('admin.mahasiswa.destroy', $row) }}" data-confirm="Apakah kamu yakin ingin menghapus mahasiswa ini?">
                                    @csrf
                                    @method('DELETE')
