<x-portal-layout :title="'Skripsi - '.config('app.name')" subtitle="Skripsi">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @if ($canManage)
                <form id="bulkDeleteSkripsiForm" method="POST" action="{{ route($routeGroup.'.bulk-delete') }}" data-confirm="Hapus data skripsi yang dicentang?">
                    @csrf
                    @method('DELETE')
    ...
    <div class="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-white/5">
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody id="skripsiListBody" class="divide-y divide-white/10">
                        ...
                        <tr class="hover:bg-white/5" data-show-url="{{ $showUrl }}">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if ($canManage)
                                        <form method="POST" action="{{ route($routeGroup.'.destroy', $row) }}" data-confirm="Hapus data skripsi ini?">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $canManage ? 6 : 5 }}" class="px-4 py-10 text-center text-emerald-100/70">Belum ada pengajuan skripsi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        (function () {
            ...
            const rows = document.querySelectorAll('#skripsiListBody tr');

            function collectRowUrls() {
                const urls = [];
                rows.forEach((row) => {
                    const a = row.querySelector('a[href*="/skripsi/"][href*="/admin/"], a[href*="/skripsi/"][href*="/dosen/"]');
            ...
            function openOneTab(url, idx, stamp) {
                const name = 'skripsi_' + stamp + '_' + idx;
                const fallback = () => {
