<x-portal-layout :title="'Detail Skripsi - '.config('app.name')" subtitle="Skripsi">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            @php
                $pdfUrl = $prefix === 'admin'
                    ? route('admin.skripsi.pdf', $skripsi)
                    : route('dosen.skripsi.pdf', $skripsi);
            @endphp
    ...
    <div class="mt-5 grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
                <div class="text-sm font-semibold">Upload File Skripsi</div>
                                ...
                                <tr class="hover:bg-white/5">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-end gap-2 flex-wrap">
                                            <a href="{{ route('files.skripsi.preview', $f) }}" target="_blank"
                                            ...
                                            <a href="{{ route('files.skripsi.download', $f) }}"
                                               class="h-9 px-3 inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
        ...
        <div class="space-y-4">
                    ...
                    <form method="POST" action="{{ $assignAction }}" enctype="multipart/form-data" class="mt-4 space-y-3">
                        ...
                        <div>
                            ...
                            <select name="dosen_pembimbing_id_2" class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                                <option value="" @selected(! old('dosen_pembimbing_id_2', $skripsi->dosen_pembimbing_id_2)) style="background-color: #0d2a23; color: #fff;">-</option>
                                @foreach ($dosenList as $d)
                                    <option value="{{ $d->id }}" @selected((string) old('dosen_pembimbing_id_2', $skripsi->dosen_pembimbing_id_2) === (string) $d->id) style="background-color: #0d2a23; color: #fff;">{{ $d->nama }}</option>
                                @endforeach
                            </select>
            ...
            <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
                ...
                @if ($skripsi->dosenPembimbing2?->nama)
                    <div class="mt-1 font-medium">{{ $skripsi->dosenPembimbing2?->nama }}</div>
                @endif
