<x-portal-layout :title="'Edit Kalender Akademik - '.config('app.name')" subtitle="Kalender Akademik">
    <x-slot:sidebar>
    ...
    <div class="flex items-center justify-between gap-3">
        <div>
            ...
            <div class="text-sm text-emerald-100/70">Perbarui kegiatan kalender akademik.</div>
        </div>
    ...
    <div class="mt-5 rounded-2xl bg-white/5 border border-white/10 p-5 max-w-2xl">
        <form method="POST" action="{{ route('admin.kalender-akademik.update', $event) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium mb-1">Judul</label>
                <input name="judul" value="{{ old('judul', $event->judul) }}" required
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', optional($event->tanggal_mulai)->format('Y-m-d')) }}" required
                           class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
                    @error('tanggal_mulai')
                        <div class="mt-1 text-sm text-red-200">{{ $message }}</div>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', optional($event->tanggal_selesai)->format('Y-m-d')) }}"
                           class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
            ...
            <div>
                ...
                <input name="kategori" value="{{ old('kategori', $event->kategori) }}"
                       class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
                @error('kategori')
                    <div class="mt-1 text-sm text-red-200">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4"
                          class="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30">{{ old('deskripsi', $event->deskripsi) }}</textarea>
                @error('deskripsi')
                    <div class="mt-1 text-sm text-red-200">{{ $message }}</div>
                @enderror
            </div>
