<x-portal-layout :title="'Jurnal Kegiatan KKN - '.config('app.name')" subtitle="KKN">
    ...
    @php
        ...
        $jurnalApproveAllUrl = $prefix === 'admin' ? 'admin.kkn.jurnal.approve-all' : 'dosen.kkn.jurnal.approve-all';
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            @if($canReview && $jurnals->where('status', '!=', 'approved')->isNotEmpty())
                <form method="POST" action="{{ route($jurnalApproveAllUrl, $kkn) }}" onsubmit="return confirm('Setujui semua jurnal kegiatan sekaligus?');" class="m-0">
                    @csrf
