<x-portal-layout :title="'Edit Daftar Hadir KKN - '.config('app.name')" subtitle="KKN">
    <x-slot:sidebar>
    ...
    @php
        ...
        $indexUrl = $prefix === 'admin' ? route('admin.kkn.absensi.index', $kkn) : route('dosen.kkn.absensi.index', $kkn);
        $updateUrl = $prefix === 'admin' ? route('admin.kkn.absensi.update', [$kkn, $absensi]) : route('dosen.kkn.absensi.update', [$kkn, $absensi]);
    @endphp

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <div class="text-xl font-semibold">Edit Daftar Hadir KKN</div>
            <div class="text-sm text-emerald-100/70">{{ $kkn->mahasiswa?->nama_lengkap }} ({{ $kkn->mahasiswa?->npm }}) • {{ $absensi->tanggal?->format('d F Y') }}</div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select id="status" name="status" required
                    ...
                    @foreach(['pending' => 'Pending', 'approved' => 'Disetujui', 'rejected' => 'Ditolak'] as $v => $l)
                        <option value="{{ $v }}" class="bg-neutral-900" @selected(old('status', $absensi->status) === $v)>{{ $l }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea id="catatan_pembimbing" name="catatan_pembimbing" rows="3"
                    ...
                    placeholder="Tulis catatan untuk mahasiswa...">{{ old('catatan_pembimbing', $absensi->catatan_pembimbing) }}</textarea>
                <x-input-error :messages="$errors->get('catatan_pembimbing')" />
            </div>
            <div class="md:col-span-2">
                <x-input-label for="keterangan" value="Keterangan Mahasiswa" />
