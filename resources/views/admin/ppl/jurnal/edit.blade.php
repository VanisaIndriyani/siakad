<x-portal-layout :title="'Edit Jurnal PPL - '.config('app.name')" subtitle="PPL">
    ...
    @php
        ...
        $indexUrl = $prefix === 'admin' ? route('admin.ppl.jurnal.index', $ppl) : route('dosen.ppl.jurnal.index', $ppl);
        $updateUrl = $prefix === 'admin' ? route('admin.ppl.jurnal.update', [$ppl, $jurnal]) : route('dosen.ppl.jurnal.update', [$ppl, $jurnal]);
    @endphp

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <div class="text-xl font-semibold">Edit Jurnal Kegiatan PPL</div>
            <div class="text-sm text-emerald-100/70">{{ $ppl->mahasiswa?->nama_lengkap }} ({{ $ppl->mahasiswa?->npm }}) • {{ $jurnal->tanggal?->format('d F Y') }}</div>
        </div>
