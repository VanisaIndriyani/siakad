                ...
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <a href="{{ route($jurnalEditUrl, [$ppl, $jurnal]) }}" class="ml-2 h-8 px-3 inline-flex items-center gap-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition text-xs text-white/90">
                                <i class="fa-solid fa-pen"></i>
                                <span class="font-medium">Edit</span>
                            </a>
                            <form method="POST" action="{{ route($jurnalDestroyUrl, [$ppl, $jurnal]) }}" onsubmit="return confirm('Hapus jurnal kegiatan ini?');" class="m-0 inline-flex">
                                @csrf
                                @method('DELETE')
                        ...
                        @if($canReview)
                            <form method="POST" action="{{ route($jurnalStatusUrl, [$ppl, $jurnal]) }}" class="mt-4 rounded-xl bg-white/5 border border-white/10 p-4">
                                @csrf
                                @method('PATCH')
