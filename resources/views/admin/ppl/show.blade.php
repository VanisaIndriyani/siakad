<x-portal-layout :title="'Detail PPL - '.config('app.name')" subtitle="PPL">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div class="min-w-0">
            <div class="text-xl font-semibold">Detail Pengajuan PPL</div>
            <div class="mt-2 flex items-center gap-2 text-sm text-emerald-100/70 flex-wrap">
                ...
                <span>•</span>
                <span class="font-medium">{{ $ppl->mahasiswa?->nama_lengkap ?: '-' }}</span>
    ...
    <div class="mt-5 grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="lg:col-span-2 space-y-4">
            <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
                ...
                <div class="mt-1 text-base font-semibold">{{ $ppl->instansi_nama }}</div>
                @if ($ppl->instansi_alamat)
                    <div class="mt-3 text-sm text-emerald-100/85 whitespace-pre-line">{{ $ppl->instansi_alamat }}</div>
                @endif
                @if ($ppl->keterangan)
                    <div class="mt-3 text-sm text-emerald-100/85 whitespace-pre-line">{{ $ppl->keterangan }}</div>
                @endif
            </div>
        ...
        <div class="space-y-4">
                ...
                <form method="POST" action="{{ $statusAction }}" class="mt-4 space-y-3">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <select name="status" class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                            <option value="approved" @selected(old('status', $ppl->status) === 'approved') style="background-color: #0d2a23; color: #fff;">Setujui</option>
                            <option value="rejected" @selected(old('status', $ppl->status) === 'rejected') style="background-color: #0d2a23; color: #fff;">Tolak</option>
                        </select>
                        <textarea name="catatan_admin" rows="4" placeholder="Catatan (opsional)"
                                  class="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-sm text-white placeholder:text-emerald-100/40 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">{{ old('catatan_admin', $ppl->catatan_admin) }}</textarea>
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.ppl.assign', $ppl) }}" enctype="multipart/form-data" class="mt-4 space-y-3">
                        ...
                        <div>
                            ...
                            <select name="dosen_pembimbing_id" class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30" required>
                                <option value="" disabled @selected(! old('dosen_pembimbing_id', $ppl->dosen_pembimbing_id)) style="background-color: #0d2a23; color: #fff;">Pilih dosen</option>
                                @foreach ($dosenList as $d)
                                    <option value="{{ $d->id }}" @selected((string) old('dosen_pembimbing_id', $ppl->dosen_pembimbing_id) === (string) $d->id) style="background-color: #0d2a23; color: #fff;">{{ $d->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="dosen_pembimbing_id_2" class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                                <option value="" @selected(! old('dosen_pembimbing_id_2', $ppl->dosen_pembimbing_id_2)) style="background-color: #0d2a23; color: #fff;">-</option>
                                @foreach ($dosenList as $d)
                                    <option value="{{ $d->id }}" @selected((string) old('dosen_pembimbing_id_2', $ppl->dosen_pembimbing_id_2) === (string) $d->id) style="background-color: #0d2a23; color: #fff;">{{ $d->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input name="nomor_sk" value="{{ old('nomor_sk', $ppl->nomor_sk) }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_sk" value="{{ old('tanggal_sk', optional($ppl->tanggal_sk)->format('Y-m-d')) }}"
                                   class="h-11 w-full rounded-xl bg-white/5 border border-white/10 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30" />
                        </div>
            ...
            <div class="rounded-2xl bg-white/5 border border-white/10 p-5">
                ...
                <div class="mt-1 font-medium">{{ $ppl->dosenPembimbing?->nama ?: '-' }}</div>
                @if ($ppl->dosenPembimbing2?->nama)
                    <div class="mt-1 font-medium">{{ $ppl->dosenPembimbing2?->nama }}</div>
                @endif
                <div class="mt-2 text-sm text-emerald-100/70">
                    SK:
                    <span class="font-medium">{{ $ppl->nomor_sk ?: '-' }}</span>
                    @if ($ppl->tanggal_sk)
                        <span>•</span>
                        <span class="font-medium">{{ $ppl->tanggal_sk->format('d/m/Y') }}</span>
                    @endif
                </div>
                @if ($ppl->sk_pembimbing_path)
                    <div class="mt-3 flex items-center gap-2 flex-wrap">
                        ...
                        <div class="text-sm text-emerald-100/70 truncate">
                            {{ $ppl->sk_pembimbing_name ?: basename($ppl->sk_pembimbing_path) }}
                        </div>
                    </div>
                @endif
            </div>
