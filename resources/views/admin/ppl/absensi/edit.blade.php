<x-portal-layout :title="'Edit Daftar Hadir PPL - '.config('app.name')" subtitle="PPL">
    ...
    @php
        ...
        $indexUrl = $prefix === 'admin' ? route('admin.ppl.absensi.index', $ppl) : route('dosen.ppl.absensi.index', $ppl);
        $updateUrl = $prefix === 'admin' ? route('admin.ppl.absensi.update', [$ppl, $absensi]) : route('dosen.ppl.absensi.update', [$ppl, $absensi]);
    @endphp

    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <div class="text-xl font-semibold">Edit Daftar Hadir PPL</div>
            <div class="text-sm text-emerald-100/70">{{ $ppl->mahasiswa?->nama_lengkap }} ({{ $ppl->mahasiswa?->npm }}) • {{ $absensi->tanggal?->format('d F Y') }}</div>
        </div>
